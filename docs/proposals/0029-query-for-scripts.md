...
[0016](0016-flag-ownership.md), where format lists are per-command surface, and
`fill` set the precedent with its own list. Also
[0026](0026-corpus-checks-are-findings.md), which grows the same `QUERY_FORMATS`
list with the `--check`-gated findings formats; the combined surface is
specified below. Also [0025](0025-query-dry-run-polarity.md), where nothing here
changes the write polarity
...
spreadsheet, and query speaks only `pretty` (for eyes) and `json` (for programs). The `--db` export covers the heavyweight case, which is opening the whole corpus
in a SQLite UI. But the common one is a single result table into Sheets or a
pandas one-liner, and today that is a `jq` incantation away.
...
one const and its error text, rather than forking a branch in the CLI dispatch. `-f csv --check` is legal: a check's rows are
still rows, the exit code carries the verdict, and the zero-row header-only output *is*
the passing gate. `COMMON_FORMATS` stays two, and `get` and `schemas` are untouched. Query comes first because its
result is the one that is already a table. `get` growing csv is a natural
follow-up when someone wants it, not a tagalong here.
...
specifies CRLF. LF is the right default for scripts and CI.   The divergence is named in the reference. A consumer that insists on CRLF,
  such as Excel double-clicking a `.csv` on Windows, then knows to re-terminate. RFC 4180 quoting
still applies: a field is quoted when it contains a comma, quote or newline, and
quotes double;
...
and not a table. The refusal lives in the CLI's format dispatch, where the format is known and
the existing error path already means exit 2. The reporter stays
presentation-only, per its own module contract. The `--db`-export-only run (no SQL) prints its summary as today under
`pretty` and `json`, and refuses `csv` the same way, because there are no rows
to shape.
...
- **CLI:** Repeatable `--param name=value` binds `value` as a **string**;
  ...
  arrays, null), which is the httpie and jq convention. Split on the first `=` or
`:=`; everything
  after is the value, so values containing `=` need no escaping.
- **API:** `QueryOptions.params?: Record<string, unknown>`. Values pass through the same `bindValue` the projection loader uses. So booleans
become `1` or `0`, and arrays and objects become JSON text. A bound parameter
then compares against a stored cell under exactly the encoding the cell got.
- **SQL.** Standard named parameters, spelled `$name`, `:name` or `@name`,
usable anywhere a
  value goes: SELECTs, `--check` gates, UPDATEs. The engine change is one call site, where `stmt.all()` grows the bound object.
That is why parameters work uniformly across read, gate and write, with no
per-path code.
...
status = $status` with the `--param` forgotten matches nothing, and returns zero
...
**A. CSV renders changes too.** Rejected. Eight change kinds share no column set, so the CSV would be a union of
mostly-empty columns. That is a shape nobody can load without reading docmeta's
source. `json` already carries changes structurally.

**B. JSON-first parameter parsing (`--param n=5` binds the number 5).** Rejected by a probe-informed footgun. The projection's columns deliberately have
no type affinity, so a bound number `2026` never equals a stored string
`"2026"`. `--param title=2026` would then silently match nothing. And quoting
your way back to a string, as `--param 'title="2026"'`, is shell-quoting hell on
two platforms. Strings are the
default because metadata is mostly strings; `:=` makes numbers deliberate.

**C. Add csv to `COMMON_FORMATS` (so `get` and `schemas` grow it too).** Rejected for this proposal. `schemas`' output is not a table. `get`'s is, but
nobody has asked. And 0016's rule is that a flag value means what its command
says, which per-command lists keep true. Recorded so `get -f csv` is a follow-up with a home, not a parity debt.
...
binds `a=b`. The typed-string spelling needs its shell quoting stated exactly, because review
caught the trap. Written bare, `--param v:="5"` has its double quotes eaten by
the shell. The program then receives `v:=5` and binds the **number**, which is
the precise inversion this mechanism exists to prevent. The reference therefore teaches `--param 'v:="5"'`, with outer single quotes
...
**5. CSV quoting round-trips through a real consumer.** The unit test writes a corpus whose titles contain commas, quotes and a newline.
It then asserts the emitted bytes parse back to the same cells under an RFC 4180
reader. That proves the escaping rather than string-matching it.
...
longer lists with commas. Both are named here so the implementation PR treats them as expected fallout, not surprises.
