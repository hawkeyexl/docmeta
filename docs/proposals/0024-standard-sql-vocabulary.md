...
[0022](0022-sql-write-back.md), stacked on their PRs. Nothing here has released.
- **Relates to:** [0023](0023-metadata-vocabularies.md), in design on its own
branch. "Vocabulary" there means the house schema families, and the DDL below is
how a corpus would one day *evolve* such a schema. Also
[0010](0010-init-and-schema-inference.md), where `schemas infer` births a schema
and this proposal grows one
...
unrealized: schema evolution and corpus migration are two hand-synchronized changes. Sara ships a stricter schema, then someone migrates the files, and CI is red for
exactly the gap between those commits. S3's whole journey is managing that gap.
...
row snapshot. A column that appears, disappears, or pairs is a DDL effect. Pairing is per-row
value equality, the same pairing DML renames use. The effect carries the
declared type, `notnull` and `dflt_value`, so nothing parses the ALTER.

**Which schema mutates.** The run's files must resolve to **one** schema set. A corpus whose overrides
split it refuses, naming the split, and the remedy is to scope the run to one
group. For DROP and RENAME, the target is the schema in the set that *declares* the
key. For ADD it is the set's single local-file schema, or `--schema <ref>` names
one. Remaining ambiguity refuses.

**Two mutation modes, decided by `classifyRef`:**

- **A builtin forks.** Built-in schemas are immutable, because the manifest pins their bytes and
`schemas:check` enforces it. So `ALTER` over `google:okf:0.1` writes a local
copy. That defaults to `./schemas/okf-0.1.local.json`, with `$id` suffixed
`+local`, and a collision refuses. It then **repoints the references**: the
config entry through the comment-preserving YAML Document API, and any in-file
`$schema` through the ordinary metadata writer. That is one writer pass, since
  a file whose `$schema` moves is already a file edit.
- **A local file is edited in place.** It is the author's hand-maintained schema. The edit
  ...
  the review surface. Version and `$id` discipline stay the author's. The preview names the file, so a
bump is a conscious follow-up. S3's semver guidance is cited rather than
enforced: additive is minor, and required, drop or rename is major.
...
null. 0022's
own text carries the pointer. The frontmatter reading wins. To a docs engineer, "set it to nothing" means "it
is not set". The corpus then reads back the way the statement left it, so `WHERE
k IS NULL` now matches. The literal `k: null` keeps a spelling, `explicit_null()`,
...
**`DELETE FROM docs WHERE …` strips the block.** Removed rows become `cleared` changes. A new writer primitive,
`stripFrontmatter(content)`, removes the fences and inner text by
`locateFrontmatter` offsets. Deleting every key would instead leave an empty
`---`/`---` shell, which is not the same thing. Fence-family formats only; element formats refuse; matching a
...
validated as relative, with no `..`, under the run's base, and not already on
disk. No other
...
refuses to invent fenced blocks, since a bare `---` means something else in
both), as do the
element formats.
...
type history, so an array value would restore from its projection as a JSON *string*. The pairing pre-pass carries the original file value verbatim, with no SQL
round-trip. It is a per-file delete of `a` and create of `b` whose SQL values
match `bindValue(original)`. The
...
The author already has a change-review mechanism, which is git. Forking is
...
**1. The array-rename corruption is what makes pairing mandatory.** Found in design. `ALTER TABLE docs RENAME COLUMN tags TO topics` without pairing
restores each file's array through the projection, where it is JSON text. With
no type history on the created column, it then writes `topics: '["a","b"]'` as a string. The pre-pass carries the original value
and never round-trips it.

**2. `DROP TABLE` is the accident-shaped spelling and stays refused.** It is one keystroke of intent away from `DELETE FROM docs`. But "delete the
table definition" would mean "delete the schema", and the destructive reading of
an ambiguous statement is the wrong default. The refusal message names `DELETE FROM docs WHERE …` for the strip and
...
`---`/`---` with nothing between, so `_present` is still 1 and `validate` still
judges an empty
...
beside the config, with `$id` `google:okf:0.1+local`. `classifyRef` still reads
the file ref as a file, and the `+local` suffix keeps the fork out of the
published namespace. An existing file at the fork path refuses rather than
overwrites.
...
(0021). Reading the assignment as removal makes the corpus read back the way the
statement left it. `explicit_null()` keeps the literal spelling for the rare
file that means `null`.
...
`--write`, not a special case per statement. That is one rule a user can hold.

**10. INSERT's format limits are the writers' limits, stated.** Markdown and MDX create blocks. AsciiDoc and RST refuse, because a bare `---` is
a transition or an open-block there, which surfaces the writer's existing
refusal. Element formats have no block to create. The
...
refuses. The same lesson turned up in miniature three more times. The config edit re-derived the config's path instead of using the one discovery
loaded. That gave the wrong or missing file under `-c`, `.yml`, or an ancestor
config. The repoint
matched only the string spelling of a `schemas:` entry and of `$schema`. And an
in-place edit left a vendored file's `integrity:` pin stale, bricking every
later run. One rule fixes all four. DDL goes through the same machinery as reading, never a
...
DROP and RENAME edited the first schema that declared the key. A sibling in the
same set that also declared or *required* it then kept failing the corpus after
a "successful" write. This repo's own docs override pairs a house schema with
`astro:starlight:0.41`, which requires `title`. DDL now loads every member of the set, and refuses when ownership is shared. The
same holds for `ADD` onto a property any member already declares, where the
silent path would have clobbered its constraints. And because SQLite accepts `INTEGER … DEFAULT 'high'`, the declared type and the
backfill value are reconciled before anything is written. One statement must not
produce a corpus that fails the schema it just gained.
