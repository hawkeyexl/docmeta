...
would publish itself. They cover the facts a docs set needs to stay
*maintained* and *machine-consumed*, which no generator defines and no
standard owns: ownership, review, audience, applicability, lifecycle,
relationships, AI provenance and guidance, and per-document quality
contracts.

The evidence base has four parts. First, a context-engineering model for
AI-ready documentation: the maturity ladder `id · type · description ·
owner` → typed applicability → eval-instrumented → drift-monitored, with
`intent`, `source-of-truth`, `risks`, and per-page sample questions.
Second, a survey of 225 writing skills' entry criteria as a metadata demand
signal. Third, the registry's own key space (19 built-ins, 220 distinct
keys once the in-flight platform schemas land). Fourth, a design walk
through the in-progress metadata contracts of three tools in the same
family (docevals, dockg, moose-tracevals), which the common vocabularies
below replace.
...
1. **Weak floors teach bad habits.** `title` and `description` are required,
   ...
   composability law (Dublin Core allows arrays, Docusaurus allows empty;
   both are override cases). All three family repos' docs gates already
   enforce this.
2. **One value is a string; many values are a list.** No per-field trivia:
   single-valued facts are plain strings, and list-valued facts accept a
...
5. **Facts live at their altitude.** `stakeholders` is page-level (who to
   consult about *this* page), distinct from the project-level
...
6. **Enumerate only what is switched-on and bounded, or published.**
   ...
   **open-enum idiom**, an `anyOf` of an advisory enum and a free string, so
...
9. **Machines propose; humans retire the provenance.** The `provenance`
   ...
   the graph block (`kg.provenance`), and in both eval schemas. It is one
...
keys are claimed at the loosest lawful definition (`keywords` down to
...
(`type: ""` would otherwise reach the kg type derivation and template
selection as a falsy key). And `type` and `language` are single strings even
...
objects and nothing looser (list members strings or objects, never bare
numbers). It is therefore the one field here that is not a `stringList`; no
claimant asks `owner` or `reviewed-by` to be anything but plain names. The
review dates are records, not freshness gates. JSON Schema cannot compare a date
to today, and the overdue-review case is pinned as *passing*; a freshness grader
reads the same `last-reviewed` field and is the thing that owns the clock.
...
sibling lists, and the contradicting page is pinned as *passing*, as the
overdue review is.
...
- **"See also" is the rendered label for `related-pages`.** The heading is
  editorial, the key is semantic, and there is no `see-also` alias: one fact
  reachable by two keys is the second surface this family exists to prevent.
  `related-pages` already accepts URLs, so a style guide that splits "See
  also" from an off-site "Learn more" is making a rendering decision over one
  field, not needing two.
...
- **`authors` moved from core to stewardship.** Core answers *what is this
  ...
  `authors` down to `stringList` for tidiness, which would break the
  compatibility rule against two documented claimants to make one schema look
  neater.
...
reverses that on purpose, with a new dividing line: **docmeta publishes
...
moose-tracevals have never shipped, so every break below is visible and
costs nothing.

**docmeta:evals:1.0.0-proposal.2**: ledger vs docevals' draft `frontmatter-0.1`.
Claims `evals` (one
...
(`success-exit-codes`, `timeout-ms`, `generated-assertion-hash`, with the
...
property at the open page root. Round 6 (`proposal.2`) added `weight`,
`target`, `runs` and `model` to entries, `weight` to the `use:` form, and
moved the prefix guard from prose into the schema; see the round-6 section.
...
gains the single-string shorthand, so every 0.8-valid document stays valid.
...
**docmeta:artifact-evals:1.0.0-proposal.2**: ledger vs moose-tracevals' draft
...
`proposal.1` kept one asymmetry, `assertion` unconditional here where the
page side lets a tool grader be its own check; round 6 removed it, and
`proposal.2` ports the page side's conditional block verbatim. The grader is
a full open enum: recommended `ai · human · command · tool-usage ·
skill-invoked · file-access · turn-count · cost · regex · json-output`, plus
the page side's `tool:*` spelling so one grader name ports across both
vocabularies; any kebab name is legal and registry-validated. In
`proposal.1` no schema conditional switched on the grader at all; in
`proposal.2` the assertion rule branches on `ai`, `human` and `command`, and
the enum stays open because an unknown name matches none of those branches.
`human` here is judged per session: every trace is new, so no verdict
caching. The 0.2 defaults (`severity: error`, skip `false`) are restored.
...
The third segment is forced by `check-builtin-schemas.mjs`: a published
schema's bytes may never change, and adding an entry is free. So the only
lawful way to fix a typo in a field `description` (and on these ids the
descriptions substantially *are* the deliverable) is to publish a new
version. With two segments the only move is `1.1`, which announces new fields
when none were added. `1.0.1` says what actually happened. PATCH is also the
one bump that can be *proved* rather than asserted: run the ladders against
both versions and require an identical verdict on every case.
...
Round 6 revised three families to `1.0.0-proposal.2`: `evals`,
`artifact-evals`, and `core`. The other six stay at `proposal.1`, because a
bump would announce a revision none of them made and leave six pairs of
byte-identical files to explain. `test/default-schema.test.ts` and
`ladders/compat-check.cjs` each carry a per-family `VERSIONS` table so a
family's next bump is still a one-line edit. The full reasoning is in the
design notes (§ Review round 6); the decisions:

- **`weight`** (both eval families, on the inline and the `use:` forms).
  Positive number, default 1. It changes how much an outcome moves an
  aggregate score and never the eval's own pass/fail, because the binary
  outcome is what SARIF, JUnit and findings baselines consume downstream.
  Zero is excluded: a weightless eval is a silent disable, and `skip` already
  means that loudly.
- **`target`** (both). What the grader receives. For a page: `body`
  (default), `raw`, or `frontmatter`. For a session: `transcript` (default),
  `last-message`, `files`, or `artifact`. Either side also accepts a
  `{source: file, path}` object naming a companion file. Distinct from
  `evidence`, which hints where to look inside what is graded; `target`
  selects the bytes, so deterministic graders honour it too. Named `target`
  rather than `focus` because it selects data, must serve graders that have
  no focus, and does not collide with `evidence`.
- **`runs` and `model`** (both). Per-eval ensemble count (1 to 50, capped
  because runs multiply cost directly) and judging model. Both, with the
  pre-existing `provider`, are constrained to `ai` evals by a conditional.
  `model` also lets an eval name a judge other than the model that wrote
  what it grades, which turns the self-preference warning `generated-by`
...
- **`assertion` became conditional in `artifact-evals`.** The page side's
  ...
  assertion; `command` requires an assertion or a command. A `tool-usage`
...
- **All nine append to `DEFAULT_SCHEMAS`** (corrected 2026-08-26 from
  ...
  change (`feat!:`) that enforces the floor every docs gate in this family
  already enforces. It also validates every other family key a page
  carries: the invented enums, the deprecation rule, the `kg` block's
  closure, the eval entry shapes. Ships with the demo video the house rules
  require of a feature. The complete list of what a bare run newly
  rejects, beyond the required pair: empty strings on core's keys; the
  DCMI array form of `language` (array `type` already fails bare runs
  today, since okf types it string); and malformed values under any key
  the other eight claim, so `lifecycle: experimental`, a typo'd `kg`
...
- The whole family therefore lands on a bare `docmeta fill` menu. Accepted
  on purpose: the default set is the teaching surface for what frontmatter
  should contain.
...
capability was lost. The compat-check probes every shared key with the other
claimants' most extreme legal values, expects only the recorded exceptions,
and exits non-zero on anything else. It is the reproducible evidence behind
principles 1 and 7.
...
7. **Where machine-production fields live.** `generated-by` and `provenance`
   ...
   default-on the practical difference shrinks to override cases, but the
...
8. **TOML frontmatter and the date fields.** `smol-toml` yields native date
   ...
   schemas PR (#117) lands. This is a sequencing dependency, not a design
   choice.
9. **What users pin, now that patch bumps are real.** Three-segment semver
   ...
   `1.x`), but that cuts against `PUBLISHED_ALIAS`, which is an exact-string
   table with no prefix rule on purpose, so that a URL naming a version that
   does not exist stays a 404 instead of resolving to something else
...
2. **The composability law was probed mechanically, not asserted.** The
   ...
   are core's non-empty floor and its single-valued `type`/`language`
   against DCMI's repeatable elements, recorded above and pinned as
   expected-rejects.
