...
- **A violation is a row, not a finding.** The output is a result table. It names no line,
  renders as no `::error` annotation on the PR, reaches no SARIF or JUnit consumer — every
  channel `validate` findings already have (0003), corpus violations lack.
- **The rule lives in the wrong file.** Every other rule this repo enforces is named in
  `docmeta.config.yaml`; the corpus rules hide in `.github/workflows/`, YAML-escaped,
  invisible to anyone reading the config to learn what the standard is.
...
0021's roadmap named this P3 and sketched the shape: a convention over result columns,
`lineFor` exposed to SQL, then named `checks:` in config, run by `validate` alongside the
schemas. This proposal is that design, stress-tested.
...
A top-level `checks:` list of `{ name, query }` mappings, parsed in `parseConfig` beside
the other keys, with the same shape discipline `overrides:` has: array guard, per-entry
mapping guard, unknown keys rejected before field checks, and an entry with a missing
...
### Rows are findings: the column convention

A check's SELECT says where each violation lives by naming its columns:

| column | | meaning |
|---|---|---|
| `path` | required | the file the finding attaches to — must be a file the run loaded |
...
Every container in the findings pipeline is per-file (`ValidationResult`), so an aggregate
rule reports by emitting one row per offending file — the `unique-slugs` check above is
the worked example: the aggregate lives in the subquery, the outer SELECT fans it back out
to files.
...
`validate` runs the checks **after its per-file loop and before `settleBaseline`** — the
...
instead of dropping each after validation; 0021 measured that holding a docs corpus in
...
`classname="docmeta.validate"` becomes a parameter.
...
**A. Status quo: gates stay in workflow files.** Rejected — the four problems above are
...
**C. Findings formats on `query --check` only, no config key.** Rejected as the whole
answer: the rules would stay in workflow YAML and `validate` would stay blind, which
forfeits the baseline ramp — the piece that makes a new rule adoptable at all. Kept as a
...
`BUILTIN_ID` (`seg(:seg)+` over `[a-z0-9._-]`, no separators, not ending `.json` —
`src/core/schema-registry.ts:207-220`) and passes through untouched. But
`check:my rules`, `check:a/b`, or `check:slugs.json` classify as **file paths** and get
...
occurrence of a recorded print. Two findings in one file that differ only in `message`
share a fingerprint (message is deliberately excluded), collapse to one entry, and any
...
`instancePath`, which is fingerprinted). The reference documents this beside the column
table; the synthesized-message fallback exists so a lazy check is still usable, not so it
is a good idea.
...
files" counting files never read) and would have to invent the result's required `format`.
A check pointing outside the corpus is a broken check — the same class as a check whose
SQL does not prepare, which is also exit 2 with the check named, not a finding.
...
does not carry — constructible from query's existing `RunContext` (`cwd`, `base`,
...
for display — never loaded. No network dependency is introduced into `validate`'s check
phase or `query`.

## Not breaking

Additive: a new config key, a new flag, new formats on `query`, no change to any shipped
behavior — `feat:`, a minor release, with the demo video the house rule requires. One
caveat stated rather than hidden: docmeta rejects unknown config keys, so a shared
`docmeta.config.yaml` that adopts `checks:` requires every consumer of that config to run
a docmeta at or above this feature. That is true of every new config key; it bites here
