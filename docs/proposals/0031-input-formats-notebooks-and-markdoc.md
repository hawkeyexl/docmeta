...
extraction. That makes the workaround look free until the corpus is mixed.
...
read. That is correct, because a silently skipped input is 0014's failure. It is
...
exactly as `mdx.ts` does today. That is a dozen lines and no new mechanism, with
`apply` included so it is writable from the first commit.
...
verbatim. The containing thing is the namespace, and here that is `metadata`. It
carries a consequence worth stating plainly rather than discovering.
`myst:frontmatter:1.10`'s flat `kernelspec` does **not** reach
`metadata.kernelspec`. That is correct, because the schema describes a Markdown
page's frontmatter. It is the gap a `jupyter:nbformat:4.5` built-in over the
...
"The convention lifts values, not structure" is what keeps a document body out
of the key set, and cell metadata is body. It is also unnameable: positionally
...
### Writing it, as a splice and never a re-serialization
...
There are 124 tracked JSON and YAML files, none of them documents, against 47
published pages. So docmeta's own corpus would become 72% CI workflows, issue
templates, and `docmeta.config.yaml` validating itself against the default
document schema.
...
is body rather than document metadata. That is the reason that survives. "We
couldn't name it" would just be an invitation to try harder.

### 5. Registering `.json`/`.yaml` would have swallowed the repository

Measured above: 124 tracked JSON and YAML files against 47 documents. That
includes `docmeta.config.yaml`, which a walk would hand to the validator to
check against the default document schema.
...
moved `.mdoc` from "already possible, skip it" to "implement". Without it the
