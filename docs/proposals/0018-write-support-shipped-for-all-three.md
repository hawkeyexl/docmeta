...
Measured against `test/fixtures/topic.dita` — DOCTYPE, wrapped attributes, LF and
CRLF — before any code was written.

What 0007 could not have known is where the difficulty actually sat. Not in
finding the value: in the line index. xmldom measures positions against a copy of
the source with line endings normalized, folding CR LF, lone CR, NEL, LINE
SEPARATOR and PARAGRAPH SEPARATOR to LF before counting. An index recognising
only LF drifts the moment a document uses any of the others — and the symptom is
not a wrong answer but a splice at the wrong offset, which is a corrupted file.

### "writing root attributes produces DTD-invalid DITA"

Also true, and this one is not answerable by being careful — only by writing
somewhere else. DITA has a designated place for document metadata:
`<prolog><metadata><othermeta/></metadata></prolog>` for topics, `<topicmeta>`
for maps. 0007 named that as "a separate project" and stopped.
...
which is the ordinary retrofit case, not an edge one. So "new values go in an
`<othermeta>`" would have added a correction beside the stale root attribute a
DITA processor actually honors: a green report over a topic that still says the
wrong thing.
...
reason. 0007 says "`title` maps to `<title>`". Measured: a `<meta name="title">`
beats `<title>` in **either** document order, so writing to `<title>` would have
landed somewhere the reader ignores. The existing comment — "the first `<title>`
wins" — means *among several `<title>` elements*, and reads like the opposite.
...
already reports byte-exact locations and `html.ts` already requested them for its
annotation columns. Re-serializing would have reflowed entities, attribute
quoting, void elements and the doctype across every file. That discipline carried
to XML and DITA unchanged, and is why an unresolvable `&nbsp;` — which the XML
reader goes out of its way to accept — survives a write.
...
- `CONTRIBUTING.md`'s write-support guidance now carries the write-back-to-source
  rule, and `html-read.ts` / `xml-read.ts` each export the precedence their
  writer aims at, so the two cannot drift.
