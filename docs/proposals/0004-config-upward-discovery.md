...
non-repo-root working directory hits it. That includes a monorepo package
script, a `make -C` target, and a pre-commit hook invoked from a subdirectory.
It also includes a CI job with a `working-directory:` key, or a developer who
just `cd docs`.
...
A local `schemas:` ref hard-errors, at exit 2, which is at least loud. A
`paths:` glob silently resolves to nothing and **exits 0**. That is a false
green again, via a different route.
...
`.git`, only `cwd` is considered, and the walk does not happen at all.
...
> `~/docmeta.config.yaml` govern any run under it. That includes this repo's own
...
> what it is today, which is `cwd` only.
...
For every user whose config is in `cwd`, which is everyone today, config-dir and
...
> never both, so there is one resolution base per run. It is
...
> `process.cwd()`. A base-relative ref would not survive that in the
> config-`paths:` case, where `configDir === base` but neither equals
> `process.cwd()`. A library caller passing an explicit `cwd` would not survive
> it at all. Guarding on `configDir !== cwd` keeps the ref string
> byte-identical for every setup that works today, so no existing report
> changes.
...
Stopping at `$HOME` was the fallback this proposal originally specified for the
no-`.git` case. It was rejected for the same reason during implementation, and
for a concrete one. This repo's own `fill.test.ts` and `cli.integration.test.ts`
...
**With no `.git` ancestor, only `cwd` is searched**, which is today's behavior,
...
### 2. No `.git`, which is the worktree case, verified relevant here

This very repo runs from `.claude/worktrees/<name>/`, where `.git` is a
**file**, not a directory. A boundary check of `isDirectory()` would walk
straight past it into the parent checkout, and could pick up the *outer*
branch's config. That is a new instance of exactly the bug CLAUDE.md already
...
A run that finds a config today keeps finding the same one, because cwd is
searched first. A run that finds none today may now find an ancestor's, which is
the entire point. But it means a repo relying on the *default* schema set while
a config exists above it will change behavior. That is a fix, not a regression,
and the new "Using …" line makes it visible. It is nonetheless a **behavior
change that warrants a minor-version note**. Per CLAUDE.md, this project is
pre-1.0 about CLI breaks anyway.
...
### 9. Interaction with 0014, where this proposal is not sufficient alone
...
1. In `test/config.test.ts`, the walk finds an ancestor config, stops at a
   `.git` **file** as well as a directory, stops at home, and `--no-config`
   suppresses it.
2. In `test/config.test.ts`, the returned `path` is absolute and its directory
   is exposed for callers.
3. In `test/resolve-schema.test.ts`, a local `schemas:` ref resolves against the
   config directory, not cwd. This reproduces defect 2A as a red test.
4. In `test/load-files.test.ts`, `paths:`/`exclude:` resolve against the config
