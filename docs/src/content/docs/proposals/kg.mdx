...
Every label field takes one string or a non-empty list without
duplicates. The three iiRDS enums stay closed because iiRDS publishes the
lists, which is the kind of authority the family's no-invented-enums rule
asks for.
...
- **The closed block is the one exception to the family's flat-keys
  ...
  open page.
- **The deeper declaration wins; the page level is the fallback.** Where
  the block and a page-level field describe the same fact (`type`,
  `concepts`, `applies-to`), the block's declaration wins, fact by fact,
  and the page-level field feeds the graph when the block is silent. When
  `kg.type` is absent, an implementer may derive it from the page's
  `type` (`how-to` → `task`, `tutorial` → `learning`, `explanation` →
