...
description: Adopt docmeta on a repo that has never had metadata validation. Measure the metadata you already have, start with a permissive schema so the build goes green on day one, wire the CI gate, then ratchet strictness over time.
---
...
Before you pick a schema, find out what the repo already has. [`docmeta schemas infer`](/docmeta/reference/cli/#schemas-infer) reads every metadata block in a docset and counts it. It is purely statistical — no network, no model, no credentials — so it is safe to run against anything, including in CI.

```sh
npx docmeta schemas infer docs/
```
...
It reports; you decide. There is deliberately no flag that turns a coverage
percentage into a `required` list — what your docset happens to contain is not
the same thing as what your standard should demand.
</Aside>
...
```sh
npx docmeta schemas infer docs/ --out ./schemas/permissive.json
```

What comes out looks like this — nothing in `required`, and a constraint only where one was observed:
...
The draft is a starting point, not a verdict — read it and cut anything that is noise rather than standard. (`--out` refuses to overwrite an existing file, so your edits are safe on a re-run, and it refuses a path `.gitignore` covers, because a schema you cannot commit is missing in CI.)
...
A file with no frontmatter block at all still gets validated. With a require-nothing schema it passes, because there is no required field to be missing. But the moment you add a field to `required`, every frontmatter-free file fails it: a blank doc has no `type`, so a rule that requires `type` flags it. This is what the `with no metadata block` count from [step 1](#step-1-take-a-coverage-reading) is for: it is exactly the number of files that will turn red on your first ratchet, and you want to know it before CI tells you.
</Aside>

## Step 3: Wire the CI gate

With a green baseline committed, add the gate now, before the standard is perfect. docmeta exits `0` when everything passes and `1` when a file fails, so any CI system can use a bare `docmeta validate` as a pass/fail step. Wiring it early means the gate starts catching regressions immediately, even while the schema is still permissive.
...
The permissive schema was only ever a starting line. Once the gate is wired, tighten the standard — but you do not have to wait for coverage before you do it. Add the field to `required`, record the existing violations once, and let docmeta fail only on new ones:
...
[`docmeta fill`](/docmeta/reference/cli/#fill) works that backlog down. For each page it resolves the same schema set the gate uses, asks an LLM to infer the properties that are missing or invalid, and writes back only the values it is confident about. Because the schema drives the prompt, the fields it proposes are exactly the ones you are about to require.

That inference is the one part of docmeta that leaves your machine, so before you point it at a whole docset, read [Run `fill` under a data-egress policy](/docmeta/set-up/fill-providers/) for what each call transmits, what the proposal cache keeps afterwards, and how to keep inference local.

Preview before you trust it:

```sh
docmeta fill docs/reference/ --dry-run
```
...
```sh
docmeta fill docs/reference/ && git diff
```
...
```sh
docmeta fill docs/reference/ --confidence 0.9 --dry-run
```
...
{/* step {"description":"The coverage probe reports what the docset has, and counts the files with no metadata block at all.","runShell":{"command":"npx docmeta schemas infer test/fixtures/infer --no-config","exitCodes":[0],"stdio":"with no metadata block"}} */}
{/* step {"description":"A frontmatter-free file passes the permissive schema (nothing is required).","runShell":{"command":"npx docmeta validate test/fixtures/no-frontmatter.md -s ./test/fixtures/dd/permissive.schema.json","exitCodes":[0],"stdio":"1 passed"}} */}
{/* step {"description":"The same file fails OKF, which requires `type` — the avalanche the permissive start avoids.","runShell":{"command":"npx docmeta validate test/fixtures/no-frontmatter.md -s google:okf:0.1","exitCodes":[1],"stdio":"must have required property 'type'"}} */}
{/* test end */}
