...
{/* step {"description":"A frontmatter-free file passes the permissive schema (nothing is required).","runShell":{"command":"node dist/cli.js validate test/fixtures/no-frontmatter.md -s ./test/fixtures/dd/permissive.schema.json","exitCodes":[0],"stdio":"1 passed"}} */}
{/* step {"description":"The same file fails OKF, which requires `type` — the avalanche the permissive start avoids.","runShell":{"command":"node dist/cli.js validate test/fixtures/no-frontmatter.md -s google:okf:0.1","exitCodes":[1],"stdio":"must have required property 'type'"}} */}
{/* test end */}
