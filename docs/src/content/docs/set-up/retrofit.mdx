...
Threshold 0.7 · 12 files · 9 fields would be written · 7 skipped
```
...
<Aside type="caution">
`fill` writes to your files by default — `--dry-run` is the preview, not the reverse. Run it on a clean working tree so `git diff` shows exactly what changed, and keep the blast radius to one folder per commit.
</Aside>

Values below the threshold are skipped and listed by name, never written with a caveat and never silently dropped. Those are the pages that genuinely need a human, and now you have a short list of them instead of the whole docset. Raise `--confidence` when you want only near-certain values:
