...
Before you pick a schema, find out what the repo already has. [`docmeta schemas infer`](/docmeta/reference/cli/#schemas-infer) reads every metadata block in a docset and counts it. It is purely statistical, with no network, no model, and no credentials, so it is safe to run against anything, including in CI.
...
It reports; you decide. There is deliberately no flag that turns a coverage percentage into a `required`
list. What your docset happens to contain is not the same thing as what your
standard should demand.
</Aside>
...
The permissive schema was only ever a starting line. Once the gate is wired, tighten the standard. You do not have to wait for coverage before you do it. Add the field to `required`, record the existing violations once, and let docmeta fail only on new ones:
...
That inference is the one part of docmeta that leaves your machine. So before you point it at a whole docset, read [Run `fill` under a data-egress policy](/docmeta/set-up/fill-providers/). It covers what each call transmits, what the proposal cache keeps afterwards, and how to keep inference local.
...
You adopted docmeta without a flag day and without a backlog of red checks blocking everyone's work.
