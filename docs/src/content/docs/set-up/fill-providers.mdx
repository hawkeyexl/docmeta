...
[`docmeta fill`](/docmeta/reference/cli/#fill) is the one command that sends your documents somewhere. Every other part of docmeta reads files and JSON Schema and stays on the machine it runs on. `fill` asks a model to infer the metadata you are missing. To do that, it has to show the model your content.
...
Deleting the cache costs nothing but a re-run. Keeping it lets you re-score proposals at a different `--confidence` for free, which is [covered in the CLI reference](/docmeta/reference/cli/#the-confidence-gate). Weigh that against a retained copy of the model's full output, and delete the cache when the retrofit is done.
