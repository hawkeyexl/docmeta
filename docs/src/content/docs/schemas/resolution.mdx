...
A `$schema` value that is anything else (a number, an object, a list with non-string entries) is a configuration error, and docmeta reports it rather than guessing. Keep it to a single reference or a flat list of references.
</Aside>

Level 2 beating config is what makes a document self-describing, and it is also the one level a repository can narrow. A repo taking pull requests from outside can say that documents may name a built-in id or a repository file but not a URL — or that their `$schema` is ignored entirely — with [`schemaTrust`](/docmeta/reference/configuration/#schema-trust). Left unset, level 2 behaves exactly as described here.
...
If a reference can't be resolved, docmeta stops with an operational error (exit code 2) rather than skipping the file or falling back to the default. This applies to an unknown built-in id, a missing schema file, and a URL that 404s or times out after the 10-second limit. Treat the schema reference as load-bearing: a broken one fails the run, which is what you want in CI.
</Aside>
