...
This add-then-ratchet pattern is the heart of tightening a standard without a flag day. The full walkthrough lives in [Roll out a new required field without breaking the build](/docmeta/set-up/new-required-field/). It covers staging the rollout across a large repo, and using config to apply the stricter schema gradually.
...
The second thing that can break a schema is the JSON Schema *dialect* you wrote it in, rather than a field you changed. JSON Schema has evolved through several versions: draft-04, draft-06, draft-07, 2019-09, and 2020-12. They aren't fully compatible, and a schema written for one dialect can fail to compile under another. docmeta handles this for you. It reads each schema's own `$schema` line, the URI at the top that names the dialect's *meta-schema*. Then it compiles that schema with the matching validation engine.
...
If a schema omits its `$schema` line, or uses one docmeta doesn't recognize, validation falls back to **2020-12**. That is the dialect of the built-in schemas.
...
This is why adopting a schema from elsewhere never forces you to rewrite it to match your other schemas. That covers an older internal standard, or a third-party schema that still targets draft-07. Drop it into the set as-is; its `$schema` line tells docmeta how to read it.
