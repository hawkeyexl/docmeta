...
*starts*, as a line and column, so docmeta rebuilds the range from that
position.
...
docmeta cannot write is reported as a per-file error, and the rest of the run
...
docmeta read the value from. That matters because `fill` also corrects values
...
Wherever docmeta reads frontmatter, it accepts three interchangeable flavors.
...
codes](/docmeta/reference/output-and-exit-codes/).
...
`article.byline`. An [`elements:` path](/docmeta/reference/element-metadata/)
reaches deeper. HTML gains `head.title` beside the flat `title` it has always
produced. See [Element metadata](/docmeta/reference/element-metadata/) for the
...
schema](/docmeta/reference/dita-schema/) describes them.
...
TOML has a **native** date-time type: an *unquoted* `published =
2024-01-15T10:00:00Z` parses as a date value, not a string. JSON Schema's
`format: date-time` only constrains `type: string`. A field typed `{"type":
"string", "format": "date-time"}` **fails** on a TOML native date, with `must be
string`. The same value written as a quoted string in JSON, YAML, or TOML
passes. If your schema expects a string date, quote it in TOML: `published =
