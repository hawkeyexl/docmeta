...
| `QueryOptions` (type) | Input to `runQuery`: the same input model plus the `sql` to run. |
| `QueryRun` (type) | What `runQuery` resolves to: result `columns` (present even for zero rows) and `rows`. |
