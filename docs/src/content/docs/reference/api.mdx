...
| `QueryOptions` (type) | Input to `runQuery`: the same input model plus the `sql` to run, optional named-parameter `params` (values encoded like stored cells: booleans as `1`/`0`, arrays and objects as JSON text), optional `schemas` naming the set a DDL statement evolves (the CLI's `-s`; a run that produces no schema-evolving effects then refuses, export-only runs included), and an optional `db` export path (which makes `sql` optional). |
| `QueryRun` (type) | What `runQuery` resolves to: result `columns` (present even for zero rows), `rows`, a mutating statement's `changes`, the `frame` describing where the run stood (what SARIF and JUnit rendering need), and — when `db` was written — its path and size. |
| `QueryChange` (type) | One thing a mutating statement changed, as a union of kinds: a cell set (`key`/`from`/`to`), a key deletion (`deleted`), a key rename (`renamedFrom`), a stripped block (`cleared`), a created file (`created`), a file move (`renamed`), a schema edit (`schema`, with the op, the type/`format`/`enum`/required intent — 0028's fields included — and any `forkedFrom`), or a DDL side effect on the governing config (`config`: a fork's `schemas:` repoint, a refreshed `integrity:` pin) — plus whether it was `written` or previewed. |
...
| `CheckConfig` (type) | One named [corpus check](/docmeta/reference/configuration/#checks): its `name` (the durable half of its findings' identity) and the SQL `query`. |
...
| `JunitOptions` (type) | JUnit-specific options: the `frame`, and the `classname` each testcase carries — `docmeta.validate` by default; `query --check` findings ship as `docmeta.query`. |
...
| `renderQueryCsv` | The `-f csv` fork of the query reporter: result rows as CSV — header always, LF endings, RFC 4180 quoting, SQL `NULL` as an empty field. |
