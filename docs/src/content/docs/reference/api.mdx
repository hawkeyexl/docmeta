...
description: Every symbol docmeta publishes from its programmatic entry point, covering command cores, schema resolution, config, cache, reporters, extractors, and result types.
---
...
docmeta publishes a programmatic entry point alongside the CLI. A Node.js or TypeScript tool can call the same cores the CLI is built on. There is no process to spawn and no stdout to parse. [Consume docmeta results](/docmeta/ci/consume-results/) shows the worked example; this page is the exhaustive list.
...
The cores throw a [`DocmetaError`](#errors) for operational problems: no inputs and no config, an unknown `--as` format, an unreadable config, an unresolvable schema. That is the same condition the CLI maps to [exit `2`](/docmeta/reference/output-and-exit-codes/#exit-codes). A validation *failure* is not thrown. It comes back as a result with `ok: false`.
</Aside>
