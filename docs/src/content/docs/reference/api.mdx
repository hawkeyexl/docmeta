...
| `QueryChange` (type) | One thing a mutating statement changed, as a union of kinds: a cell set (`key`/`from`/`to`), a key deletion (`deleted`), a key rename (`renamedFrom`), a stripped block (`cleared`), a created file (`created`), or a file move (`renamed`) — plus whether it was `written` or previewed. |
