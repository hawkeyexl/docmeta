...
| `QueryOptions` (type) | Input to `runQuery`: the same input model plus the `sql` to run, optional named-parameter `params` (values encoded like stored cells: booleans as `1`/`0`, arrays and objects as JSON text), optional `schemas` naming the set a DDL statement evolves (the CLI's `-s`; a statement that runs no DDL then refuses), and an optional `db` export path (which makes `sql` optional). |
