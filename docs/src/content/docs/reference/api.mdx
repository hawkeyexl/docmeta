...
| `QueryChange` (type) | One cell a mutating statement changed: `file`, `key`, file-space `from`/`to` (or `deleted` for a key removal), and whether it was `written` or previewed. |
